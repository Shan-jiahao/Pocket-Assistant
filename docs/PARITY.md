...
| --- | --- | --- | --- |
| Pocket Assistant | The iPhone-only `Pocket助手` target reuses this table's BLE → SoftAP → UDP/DUML, camera SET, gimbal, and safety behavior. It must not fork protocol tables or HeadTrack control. | This focused product has Chinese `设备 / 头追 / 拍摄` navigation and no monitor/media surface. Capture includes compact record and gimbal controls; Head Track includes a calibrated-only, foreground Pocket Mode. Its iOS-only Lock Screen/Dynamic Island control strip shows last-known Pocket state plus red record/stop after system authentication and a fresh link check; its calibration action opens Head Track for current-motion checks. It does not keep the session or head tracking alive. Light/dark appearance follows iOS. Android parity is outside the initial extraction. | **physical iOS** |
...
| Head tracking | iOS: Controls **Head Tracking (Experimental)** (off by default). AirPods with motion. **HeadTrack Control** is a video-free control cover with AirPods/Pocket/Datalink/Gimbal status, live head/gimbal/target yaw-pitch, gimbal recenter before calibration, Apple's supported-headphone help, Fast/Standard/Gentle presets, and Sensitivity/Dead Zone/Smoothness/Max Speed. **Calibrate Head Lock** is shared forward — that head pose and that gimbal pose are zero. The App-level motion bridge keeps driving when the live view is covered. Pocket Assistant keeps its foreground screen awake while head tracking is calibrated, and its Lock Screen/Dynamic Island Live Activity reports preparation, active, or safely paused status; it is not a background control path. Stick throws through the existing 25 Hz path until live `0x04/0x05` matches the configured target; Pocket has no angle SET. Gimbal Fast + tilt unlocked at calibrate. Roll is shown, not driven. STOP clears SET. AirPods/Pocket/datalink loss, 250 ms motion timeout, and app inactive/background immediately rest the stick and clear SET. Chip/stick/gamepad win while thrown. Toast if IMU is missing or the head is moving at calibrate. | Android has no AirPods IMU — no Controls row | **physical** iOS |
...
| Diagnostics | Operator Setup → System **and** Connection setup (first pair) → **Share Diagnostics** (redacted report). Journal in app documents. Reports label the current header separately from the cross-launch rolling journal. No analytics upload. | iOS copies a compact paste on screenshot for TestFlight feedback (Apple cannot attach files to that form). Android has no TestFlight screenshot hook — Share only. MetricKit is iOS and is labeled historical because delivery can describe an earlier build. | **physical** both |
