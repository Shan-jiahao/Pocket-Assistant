...
| --- | --- | --- | --- |
| Pocket Assistant | The iPhone-only `Pocket助手` target reuses this table's BLE → SoftAP → UDP/DUML, camera SET, gimbal, and safety behavior. It must not fork protocol tables or HeadTrack control. | This focused product has Chinese `设备 / 头追 / 拍摄` navigation and no monitor/media surface. Capture includes compact record and gimbal controls; Head Track includes a calibrated-only, foreground Pocket Mode. Light/dark appearance follows iOS. Android parity is outside the initial extraction. | **physical iOS** |
